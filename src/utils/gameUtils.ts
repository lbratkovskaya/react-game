...
export const generateColorIndex = (): number => (Math.floor(Math.random() * COLORS.length)
  % COLORS.length) + 1;
...
export const generateGameFieldState = (
  ...
  const result: number[][] = (currentFieldState.length === 0) ? getZerosMatrix(fieldSize)
    : [...currentFieldState];
...
const isInMatrixBounds = (x: number, y: number, matrixLength: number): boolean => x >= 0
&& y >= 0 && x < matrixLength && y < matrixLength;
...
const processPath = (
  ...
  const tupleToNext = [[0, -1], [-1, 0], [0, 1], [1, 0]].find((tuple) => {
    ...
    return isInMatrixBounds(nextX, nextY, matrix.length)
      && matrix[nextX][nextY] === currentMark - 1;
  });
  ...
  while (queue.length > 0) {
    ...
    [[0, -1], [-1, 0], [0, 1], [1, 0]].forEach((tuple) => {
      ...
      if (isInMatrixBounds(nextX, nextY, matrix.length) && matrix[nextX][nextY] === null) {
        queue.push({ x: nextX, y: nextY, mark: nextMark });
      }
    });
  }
  return null;
...
const checkDiagonalLeftToRight = (fieldSize: number, matrix: number[][]): [number, number][] => {
  ...
  for (let row = 0; row < fieldSize; row += 1) {
    for (let col = 0; col < fieldSize; col += 1) {
      tempResult = [];
      for (let i = 0; i < fieldSize; i += 1) {
        if (col + i < fieldSize - 1
          && row + i < fieldSize - 1
          && matrix[row + i][col + i] > 0
          && matrix[row + i][col + i] === matrix[row + i + 1][col + i + 1]) {
